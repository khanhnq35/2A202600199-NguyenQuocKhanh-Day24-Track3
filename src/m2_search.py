...
class BM25Search:
    ...
    def index(self, chunks: list[dict]) -> None:
        """Build BM25 index from chunks using parallel processing."""
        ...
        with ThreadPoolExecutor() as executor:
            segmented_texts = list(executor.map(segment_vietnamese, [c["text"] for c in chunks]))
            
        self.corpus_tokens = [text.split() for text in segmented_texts]
